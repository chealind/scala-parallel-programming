...
class BarnesHutSuite {
  ...
  @Test def `'SectorMatrix.+=' should add a body at (112,115) to the correct bucket of a sector matrix of size 96 (2pts)`(): Unit = {
    val body = new Body(3, 112, 115, 0.1f, 0.1f)
    val boundaries = new Boundaries()
    boundaries.minX = 1
    boundaries.minY = 1
    boundaries.maxX = 97
  ...
  @Test def `'SectorMatrix.combine' should combine two matrix`(): Unit = {
    val body1 = new Body(5, 25, 47, 0.1f, 0.1f)
    val body2 = new Body(3, 112, 115, 0.1f, 0.1f)
    val boundaries = new Boundaries()
    boundaries.minX = 1
    boundaries.minY = 1
    boundaries.maxX = 97
    ...
    val resMatrix = sm1.combine(sm2)

    val res1 = resMatrix(2, 3).size == 1 && resMatrix(2, 3).exists(_ == body1)
    val res2 = resMatrix(7, 7).size == 1 && resMatrix(7, 7).exists(_ == body2)
    assert(res1, s"Body not found in the right sector")
